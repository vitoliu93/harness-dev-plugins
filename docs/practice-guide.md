...
```
干活 ship/orchestrate → 捕获 [LEARN] hooks → 收盘 debrief → 体检 skill-atlas → 铸造 skill-forge
        ↑                                     │
        └──────── 下次会话更便宜更准 ←────────┘
```
...
- **改任何 description** → 立刻跑该技能的 trigger eval(skill-atlas §3 有命令);没夹具的技能改完描述,顺手补一份(抄 `skills/use-agents/evals/`)。
- **月度**一次 "skill 体检"。
