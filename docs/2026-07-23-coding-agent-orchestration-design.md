...
- **盈亏下限**（2026-07-24 修订：token/diff 体量的事前预估极不准，vito 裁决尽量避免——
  派发前只用可观察信号，量值降级为账本事后审计）：① 类型默认（config/ops 整类贴盈亏线，
  默认不委派）；② 免勘察测试（不用勘察就能说出改哪个文件哪个函数 → 自己做）；
  ③ spec 副产品信号（host 反正要写 spec，写的过程即测量——spec 里实际写出了 diff /
  写 spec 比改还慢 → 当场收回）。spec:diff 0.3 与 10k 产出线均记账本事后指标，
  用于回头收紧上述三信号；总账仍按整个 job 判定（Σ spec+review+重试摊销 vs host 自做），
  防止切小绕开。
...
0. **【已完成 2026-07-24】30 天 session 回测交叉验证**（vito 2026-07-23 补）：用 ccobs 账本
   ...
   **回测结论**（窗口 06-24→07-24，蒸馏覆盖 577/820 claude-code 主会话；口径 caveat：
   ...
   - **可委派占比**：编码会话中体量在盈亏线上（≥10k out tokens）的占 64.6%；再按
     ...
     预想窄"成立，但池子足够撑试点。
   - **盈亏平衡下限**：35.4% 的编码会话 <10k out tokens（均值 ~17 turns）——此档
     spec+验收成本必然倒挂。（2026-07-24 修订：10k 不作派发前闸门——那要求预测产出，
     预测极不准；改作**账本事后审计线**，派发前判断走 §4.1 修订后的三个可观察信号。）
