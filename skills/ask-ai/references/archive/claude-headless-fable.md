# [ARCHIVED] Claude headless on fable — ultra mode's original engine

> **Archived 2026-06-13 — fable is temporarily suspended.** This was the live
> `ultra`-mode engine guide before the suspension. It is kept verbatim (not
> deleted) so ultra can be reverted the moment fable returns.
>
> **To restore:** move this file back to `../claude-headless.md` (overwriting the
> opus-4.8 stand-in), and in `SKILL.md` + `agents/ai-second-opinion.md` revert
> ultra's Claude leg from `--model opus --effort max` to `--model fable`.
