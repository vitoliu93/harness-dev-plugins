...
- **第一道硬门（必要非充分）**：能写出机器可判的验收命令。写不出 =
  藏着判断，host 自做。
- 简单任务不委派——盈亏下限初值（2026-07 回测）：预估产出 <10k tokens
  收回自做（30 天 35% 编码会话在此线下）。探索性工作、强耦合仓（跨模块
  不变量）是 host 主场；跨项目长任务是最佳甜区——前提是契约先行冻结。
- spec:diff 预估比 > 0.3（回测间接一致性通过，直接校准待 job_ledger）→
  不委派；按整个 job 总账判定（Σ spec+review+重试摊销），防切小绕开。
