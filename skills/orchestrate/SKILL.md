...
- **第一道硬门（必要非充分）**：能写出机器可判的验收命令。写不出 =
  藏着判断，host 自做。
- 简单任务不委派（盈亏下限）；探索性工作、强耦合仓（跨模块不变量）是
  host 主场。跨项目长任务是最佳甜区——前提是契约先行冻结。
- spec:diff 预估比 > 0.3（默认值，待账本校准）→ 不委派；按整个 job
  总账判定（Σ spec+review+重试摊销），防切小绕开。
...
L 档追加契约/不变量/naive_failure_mode/预算。context pack 三层拆分
