...
```bash
REVIEW_SKILL_DIR="<absolute path of the directory containing the loaded skill-review/SKILL.md>";
...
Portfolio checks can be disabled fleet-wide in `$SKILLS_ROOT/skill-review/policy.json`
...
Read [call-site.md](../call-site.md). Orphan/⚠ → wire or exempt (monthly hygiene: context-audit, skill-review).
...
SKILLS_ROOT="$REVIEW_SKILL_DIR/..";
DB=${CCOBS_DIR:-$HOME/.claude/observability}/obs.db;
bun "$SKILLS_ROOT/skill-forge/scripts/skill_usage.ts" --db "$DB" --aliases "$SKILLS_ROOT/skill-review/aliases.json" --days 30
```
