...
`.schema sessions` 复核):
    ...
    sqlite3 -header ~/.claude/observability/obs.db \
      "SELECT date(s.started_at) d, o.task_type, o.outcome, substr(o.summary,1,120) s
...
- **时间**:近因优先已由 ORDER BY 保证;结果里 >90 天的行标 ⚠(今天日期在上下文里,
  自己比一下即可)。observations 全是 main 会话、含 claude-code/codex/droid/grok/
  opencode 五源——先例不分源,不必按 source 过滤。
