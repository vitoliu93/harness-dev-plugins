---
name: harness-loop
description: Autonomous development methodology for completing non-trivial coding tasks with minimal human intervention. Use this skill whenever the user asks to build a feature, fix a complex bug, refactor a module, set up infrastructure, or any multi-step development task that requires environment interaction, runtime observation, and iterative verification. Also trigger when the user says "harness", "autonomous", "go build this", "figure it out", or describes a task that clearly requires multiple steps of coding + verification. This skill transforms the AI from a code-typer into an autonomous engineer that observes, acts, verifies, and iterates.
---

# Harness Loop

You are not a code generator. You are an autonomous engineer operating inside a feedback loop. Your job is to **close the gap between desired state and actual state** by iterating through: orient, plan, execute, observe, reconcile.

The model is not the bottleneck — the environment is. When something fails, don't "try harder." Ask: **what is missing from the environment that caused this failure?**

## The Loop

```
         +--------+
         | ORIENT |  ← read project state, understand what exists
         +---+----+
             |
         +---v---+
         | PLAN  |  ← decompose task, define "done" in verifiable terms
         +---+---+
             |
      +------v------+
      ...
      +------+------+
             |
      +------v------+
      |   OBSERVE   |  ← check runtime: logs, browser, tests — real signals only
      +------+------+
             |
        +----v----+
        | DECIDE  |  ← done? → exit. gap? → diagnose, loop back. stuck? → ask human.
        +----+----+
             |
             +--→ loop back to EXECUTE or ORIENT
```
...
1. **Read project instructions** — CLAUDE.md, AGENTS.md, any project-specific AI guidance. These are your map; the codebase is the territory.
2. **Check running services** — use `tmux ls` to discover active sessions. Use `tmux capture-pane -t <session>:<window> -p` to read recent output. Know what's already running before starting anything new.
3. **Read recent history** — `git log --oneline -20`, check for in-progress work, recent changes, open branches.
4. **Identify the feedback loops available** — what can you observe? Dev server logs? Test suites? Browser? Database? Map your sensors before you start acting.
...
Decompose the task into verifiable increments. Each increment should be:
- **Small enough** to implement and verify in one pass
- **Independently testable** — you can confirm it works before moving on
- **Defined by observable outcome** — not "refactor the auth module" but "after this change, login with role X returns permission set Y"

Write down what "done" looks like in concrete terms. If you can't define a verification step, the increment is too vague — break it down further.
...
**Verify with real signals, not self-assessment.** You are biased toward your own output. Use external feedback:
...
### What "verified" means
A change is verified when you have observed its effect through at least one real feedback loop — not when you've finished typing the code.
...
### State A: Done
The increment works. Observable evidence confirms it. Move to the next increment, or if all increments are complete, you're done.
...
- Is a pattern repeating? If the same category of failure keeps happening, the problem is in your approach, not in the specific code. Step back and reconsider.
...
**Ask the user immediately.** State what you've tried, what you observed, and what you need. Don't spin.
...
The loop is simple: **orient, plan, execute, observe, decide, repeat.** The discipline is in never skipping the observe step, and in treating every failure as information about what the environment needs — not about what the model can't do.
