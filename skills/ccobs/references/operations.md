...
- Model routing: `${CCOBS_DIR}/llm.json`，平表 `{"<场景>": "<provider>/<model>[:思考档]"}`。`pi-call.ts` 是唯一的读取者，插件里不写死任何模型；缺文件就跳过。场景键：`default`、`distill`、`rollup`、`recall`、`prompt-forge`、`skill-style-review`、`vision`。认证归 pi，不归这里。独立 agent 的渠道来自 `${AGENTS_CONFIG:-$HOME/.config/agents/agents.json}`。
