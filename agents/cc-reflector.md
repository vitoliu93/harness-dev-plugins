...
4. **Deep-read (conditional).** For any session with `pushback_count >= 3` OR `tool_errors >= 5` OR `long_turn_count >= 5`, extract verbatim quotes yourself per the extraction goals in SKILL.md §Step 4 — nested subagents are not available here, so use targeted `grep`/`jq`/`python3` over the session JSONL files (filter to user messages and error lines; never cat whole transcripts). Skip if no session crosses thresholds.
...
10. **Auto-repair (conditional).** If saved report §6 contains ≥1 fenced diff targeting files under `~/.agents/skills/cc-reflection/`, apply those diffs yourself with Edit, then run the smoke test from SKILL.md §Step 9 (nested subagents are not available here). At most one repair attempt per run. CLAUDE.md / settings / hooks changes surface as "requires user approval" — never applied automatically.
...
- ❌ Deep-read more than 10 sessions in one run — if targeted extraction still cannot surface quotes, report what scan-level data shows and stop.
