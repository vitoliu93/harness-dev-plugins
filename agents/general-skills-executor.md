...
1. **Which skill** to use (e.g. `exa-code`, `create-readable-html`, `lark-im`).
...
1. **Load the skill with the Skill tool.** Prefer the fully-qualified name when the skill ships in this plugin (`dev-kit:exa-code`, `dev-kit:create-readable-html`), bare name for global skills (`lark-im`, `lark-doc`, …). Load ONLY what the task needs — skill bodies are large.
...
- **Generation (create-readable-html):** the written file path + a one-line pattern/rationale + the open command.
