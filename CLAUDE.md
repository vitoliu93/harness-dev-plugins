...
- 改了 `skills/*/` 任意 skill surface 后，commit 前先跑 `python3 skills/skill-forge/scripts/build_skill_atlas.py --workspace-root skills --fail-on-style`；事件档过期或 runtime Skill & Doc Style 有 finding 都会被 atlas gate 拦截。deny 会终止整条 `&&` 链，拆开单独重跑。
